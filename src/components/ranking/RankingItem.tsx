...
export function RankingItem({
  ...
  const gridClass = compact
    ? "grid-cols-[42px_1fr_auto_auto] gap-2 p-2.5"
    : "grid-cols-[64px_1fr_auto_auto] gap-4 p-4";
  const rowGridClass = compact
    ? "grid-cols-[42px_1fr_auto] gap-2"
    : "grid-cols-[64px_1fr_auto] gap-4";

  return (
    <div
      className={`ranking-item group grid items-center rounded-lg border transition duration-300 ${
        active
    ...
    >
      <button
        type="button"
        onClick={onSelect}
        className={`col-span-3 grid w-full items-center text-left ${rowGridClass}`}
      >
        <span
        ...
        <div className="min-w-0">
          ...
          <RapperLabelStrip rapper={rapper} variant="compact" inverted={active} />
          <div className="mt-1">
            <FondnessStat
